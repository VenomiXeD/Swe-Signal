...
import venomized.mods.swsignal.create.tracks.ATCController;
import venomized.mods.swsignal.data.SwSignalLang;

public final class SwAllBlocks {
    // public static final DeferredRegister<Block> BLOCKS = DeferredRegister.create(ForgeRegistries.BLOCKS, SwSignal.MOD_ID);

    public static final BlockEntry<BlockTrainConfig> BLOCK_TRAIN_CONFIG = SwSignalCore.REGISTRATE.get().block("train_config", BlockTrainConfig::new)
            .blockstate((ctx, prov) -> {
    ...
    public static final BlockEntry<BlockATCController> BLOCK_ATC_CONTROLLER = SwSignalCore.REGISTRATE.get().block("atc_controller", BlockATCController::new)
    ...
    public static <T extends Block> BlockBuilder<T, Registrate> modelledBlock(String name, NonNullFunction<BlockBehaviour.Properties, T> blockCreator) {
        return SwSignalCore.REGISTRATE.get()
                ...
                .blockstate((blockTDataGenContext, registrateBlockstateProvider) -> {
                    ...
                    SwSignalCore.LOGGER.info("modelled block path: {}", path);
    ...
    public static <T extends Block> BlockBuilder<T, Registrate> signalBlock(String assetType, String nation, String name, NonNullFunction<BlockBehaviour.Properties, T> blockCreator) {
        ...
        return SwSignalCore.REGISTRATE.get()
                ...
                .lang("(%s) %s".formatted(venomized.mods.swsignal.data.SwSignalLang.fromISO639_1(nation), properName))
                .properties(prop -> BlockBehaviour.Properties.of().destroyTime(1f))
                .blockstate((blockTDataGenContext, registrateBlockstateProvider) -> {
                    //if (blockTDataGenContext.get() instanceof Sw45DegreeBlock) {
